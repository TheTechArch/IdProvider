...
| Method & path | Purpose |
|---|---|
| `GET /Authorize` | Renders the test login form (404 if disabled). |
| `POST /Authorize` | Validates shared password → Tenor gate → issues code & redirects. Rate-limited per IP. |
...
```bash
cd src/IdProvider
dotnet run --project IdProvider/IdProvider.csproj
```
...
```bash
cd src/IdProvider
dotnet build IdProvider.sln
dotnet test  IdProvider.sln
```

Unit tests (`IdProvider.Tests`) cover the load-bearing invariant: the synthetic
